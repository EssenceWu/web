...
function popup(data, callback) { //调用例子dom.popup({txt:'xxxxxx',btn:2})
    ...
    if (data.btn == '2') {
        ...
        btn = '<span class="popup_false" id="popup_false" onclick="javascript:location.href=&quot;/luohujisuanqi_detail.php?page_id=2&quot;">查看积分详情</span><span class="popup_true" id="popup_true">' + txt_true + '</span>';
    } else {
        btn = '<span class="popup_true" style="margin: auto;width:55%;" id="popup_true">我知道了</span>';
    }

    var html = '<div class="loading_line" id="popup_pub" style="visibility: visible;">' +
        '<div class="box-middle">' +
        '<div class="popup_box fs_13" ' + style + '>' +
        '<div class="popup_title font_ff5050">' + txt_title  +'</div>' +
        '<div class="popup_txt" ' + txt_style + '><div>' + data.txt + '</div></div>' +
        '<div class="popup_btn">' + btn + '</div>' +
        '</div>' +
        '</div></div>';
    $("body").append(html);
    $('#popup_true,#popup_false').bind('click', function(e) {
        e.preventDefault();
        $('#popup_pub').hide();
        if (callback) {
            callback();
...
//经过公司显示相关信息
$(".headnav-r").mouseenter(function() {
    $('.company-info').show();
});
$(".headnav-r").mouseleave(function() {
    window.timer = setTimeout(function() {
        $('.company-info').hide();
    },
    200);
});
$(".company-info").on("mouseenter",function() {
...
$('.customer').click(function(){
  $(this).hide();
  $('.customer_iframe').show().html('<iframe src="http://api.dashebao.com/static/webkefu.html" ></iframe>');
});
$(document).mouseup(function(e){
var _con = $('.customer ');   // 设置目标区域
if(!_con.is(e.target) && _con.has(e.target).length === 0){ // Mark 1
  _con.show();
  $('.customer_iframe').hide();
}
});