    ...
    get_yzm:function(){
      ...
      $("#get_yzm").bind("click",function () {
          ...
          if (!phoneNum.match(myreg)) {
            ...
            return false;
              ...
              var sendCode = setInterval(function () {
                  if (wait == 0) {
                      ...
                      $this.removeClass("btn-gray").addClass("btn-blue").val('获取验证码');
                      wait = 60;
                      clearInterval(sendCode);
                  } else {
                      $this.removeClass("btn-blue").addClass("btn-gray").val(wait + '秒');
           ...
           $.ajax({
              type: "post",
              url: "/Salary/CompanySalaryStaffAccount/identifyCode",
